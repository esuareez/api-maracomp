...
} from 'src/inventorymovement/schema/inventorymovement.schema';
...
export class ComponentService {
  ...
  async update(id: string, component: any) {
    return await this.componentModel
      .findByIdAndUpdate(id, component, { new: true })
      .exec();
  }
  ...
  async findOne(description: string, unit: string) {
    return await this.componentModel.findOne({ description, unit }).exec();
  }
}
